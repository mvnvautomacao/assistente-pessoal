...
export type Interpretation =
  ...
  | {
      ...
      to_category: string;
      scope: "today" | "last_n" | "from_category";
      n?: number;
      category?: string;
    }
  | { type: "undo" }
...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    scope: {
      type: "string",
      enum: ["today", "last_n", "from_category"],
      description:
        "So para type=bulk_recategorize: qual conjunto de gastos muda de categoria. 'today' = os de hoje. 'last_n' = os N mais recentes (preencha 'n'). 'from_category' = todos os que estao numa categoria (preencha 'category').",
    },
    payment_method: {
      type: "string",
      description:
