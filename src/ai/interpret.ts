...
export type Interpretation =
  | { type: "expense"; amount: number; description: string; category?: string; date?: string; payment_method?: string }
  | {
  ...
  | { type: "reminder"; message: string; due_at: string; advance_minutes?: number }
  ...
  | { type: "set_bill_alert"; description: string; day_of_month?: number; interval_days?: number }
  | { type: "list_bill_alerts" }
  | { type: "remove_bill_alert"; query: string }
  | { type: "edit_bill_alert"; query: string; new_name?: string; day_of_month?: number; interval_days?: number }
  ...
  | { type: "list_expenses"; date?: string; days?: number; date_start?: string; date_end?: string }
  | { type: "total_last_list" }
  | { type: "delete_expense"; list_ref?: number; query?: string }
  | {
      type: "edit_expense";
      list_ref?: number;
      query?: string;
...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    new_name: {
      type: "string",
      description: "So para type=edit_bill_alert: o NOVO nome do alerta, SE o usuario pediu pra renomear.",
    },
    to_category: {
      type: "string",
    ...
    date_start: {
      type: "string",
      description: "Data ISO 8601 (so a data) de inicio do intervalo, so para type=list_expenses com intervalo de dias ou type=bulk_recategorize com scope='period' quando o usuario deu um intervalo exato (ex: 'gastos de 10 a 20 desse mes').",
    },
    date_end: {
      type: "string",
      description: "Data ISO 8601 (so a data) de fim do intervalo (inclusive), so para type=bulk_recategorize com scope='period' ou type=list_expenses com intervalo de dias, usado junto com 'date_start'.",
    },
    scope: {
      type: "string",
    ...
    day_of_month: {
      type: "number",
      description:
        "Dia do mes (1 a 31) em que o gasto fixo deve ser lancado automaticamente (type=set_recurring_expense), ou em que o alerta deve avisar (type=set_bill_alert, SO quando a recorrencia for por dia fixo do mes -- se for por intervalo de dias, use 'interval_days' em vez desse campo).",
    },
    interval_days: {
      type: "number",
      description:
        "So para type=set_bill_alert, quando o usuario pede recorrencia por INTERVALO de tempo em vez de um dia fixo do mes (ex: 'comprar ração a cada 45 dias' -> 45, 'trocar o filtro a cada 2 meses' -> 60, 'de 15 em 15 dias' -> 15). Nao preencher junto com 'day_of_month' -- use um ou outro, nunca os dois.",
    },
    likely_intent: {
      type: "string",
    ...
    advance_minutes: {
      type: "number",
      description:
        "So para type=reminder: preencha SE (e so se) o usuario pediu explicitamente pra ser avisado com antecedencia (ex: 'me avise 20 minutos antes', 'avisa 10 min antes da consulta') -- lembrete normal nao suporta isso (so evento suporta, via reminder_minutes), entao o sistema usa esse campo pra perguntar ao usuario se quer criar como evento em vez de lembrete. Sem mencao a antecedencia, deixe vazio.",
    },
    days: {
      type: "number",
      description:
