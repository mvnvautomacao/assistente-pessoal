...
export type Interpretation =
  ...
  | { type: "edit_event"; query: string; new_start: string }
  | { type: "reminder"; message: string; due_at: string }
  | { type: "edit_reminder"; query: string; new_due_at: string }
...
const ACTION_SCHEMA = {
  ...
  properties: {
    ...
    new_start: {
      type: "string",
      description:
        "Nova data/hora ISO 8601 de inicio (so para type=edit_event), no horario local de Brasilia (nao UTC). Inclua o offset explicito '-03:00' no final, igual o campo 'start'.",
    },
    new_due_at: {
      type: "string",
      description:
        "Nova data/hora ISO 8601 em que o lembrete deve ser enviado (so para type=edit_reminder), no horario local de Brasilia (nao UTC). Inclua o offset explicito '-03:00' no final, igual o campo 'due_at'.",
    },
    list_ref: {
      type: "number",
      description:
...
const EXTRACT_DATETIME_TOOL: Anthropic.Tool = {
  ...
  description: "Extrai a data/hora que o usuario quis dizer numa frase livre, em ISO 8601 com offset -03:00 (horario de Brasilia).",
  input_schema: {
    type: "object",
    properties: {
      datetime: {
        type: "string",
        description:
          "Data/hora ISO 8601 com offset '-03:00' explicito (ex: '2026-09-20T16:00:00-03:00'). Deixe vazio/omita se a mensagem nao deu pra entender nenhuma data ou hora.",
      },
    },
    required: [],
  },
...
// frase livre (ex: "na verdade e sexta as 16h") em vez de so "sim"/"nao" — mesma
// ideia do extractCategoryFromAnswer, mas pra data/hora de evento/lembrete.
export async function extractDateTimeFromAnswer(answerText: string): Promise<string | null> {
  const now = spNowFormatter.format(new Date()).replace(" ", "T");
  const response = await anthropic.messages.create({
    model: "claude-haiku-4-5-20251001",
    max_tokens: 150,
    system: `Data/hora atual: ${now}-03:00, horario de Brasilia (America/Sao_Paulo, UTC-3 o ano todo, sem horario de verao). Converta a data/hora que a mensagem do usuario menciona pra ISO 8601 com offset '-03:00' explicito, com base NESSA data/hora atual.`,
  ...
  const input = toolUse.input as { datetime?: string };
  return input.datetime?.trim() || null;
}
